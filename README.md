...
- **`src/plot_particle.py`** - Particle concentration decay and emission rates; re-exports boxplot/comparison functions
- **`src/plot_particle_boxplots.py`** - Temperature-axis and shower-head E_total/β/E/p boxplots
- **`src/plot_comparison.py`** - Categorical comparison boxplot families (door, fan, spray pattern, mannequin, head type)
...
**Format:** `MMDD_W##[_ShowerHead[_SprayPattern]][_Mannequin]_BathDoorXxx_BdrmDoorXxx[_Fan]_RNN`
...
| `SprayPattern` | Spray pattern; varies by head type | `Wide`, `Narrow`, `Mid`, `rainfall`, `12Nozzle`, `SingleWide` |
| `Mannequin` | Appended only when a mannequin was present | `Mannequin` |
| `BathDoorXxx` | Bathroom-to-bedroom door position | `BathDoorOpen`, `BathDoorClosed`, `BathDoorAjar` |
| `BdrmDoorXxx` | Bedroom-to-rest-of-house door position | `BdrmDoorOpen`, `BdrmDoorClosed`, `BdrmDoorAjar` |
| `Fan` | Appended only when the bath fan ran during the test | `Fan` |
| `RNN` | Replicate number | `R01`, `R02`, `R03`, etc. |

**Examples:**
- `0115_W48_BathDoorOpen_BdrmDoorClosed_R01` - January 15, 48 °C, standard head, replicate 1
- `0122_W11_BathDoorOpen_BdrmDoorClosed_R03` - January 22, 11 °C, standard head, replicate 3
- `0224_W52_Pepco_Wide_BathDoorOpen_BdrmDoorClosed_R01` - February 24, 52 °C, Pepco head, wide spray
- `0309_W40_Pepco_Narrow_BathDoorOpen_BdrmDoorClosed_R01` - March 9, 40 °C, Pepco head, narrow spray
- `0311_W40_Pepco_Narrow_Mannequin_BathDoorOpen_BdrmDoorClosed_R01` - March 11, mannequin present
- `0412_W40_BathDoorClosed_BdrmDoorClosed_R01` - April 12, 40 °C, bathroom door closed
...
**Test Parameter Timeline (FilterWand and Used shower heads, near 38 °C):**
- W38 (38 °C), FilterWand: Installed after Pepco narrow/mannequin phase
- W38 (38 °C), Used-rainfall / Used-12Nozzle / Used-SingleWide: Sequential used-head comparisons
...
This software is in the public domain and provided "as is" according to the [LICENSE.md](LICENSE.md) file. It was developed by employees of the National Institute
