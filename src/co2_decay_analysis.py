...
from scripts.plot_utils import plot_lambda_summary  # noqa: E402
...
def plot_co2_decay_event_analytical(
    ...
    """
    Plot CO2 decay data with analytical fit for a single injection event.

    Creates a two-panel figure:
        1. Top panel: CO2 concentrations vs time with exponential fit
        2. Bottom panel: Linearized plot (y vs t) with regression line

    Parameters:
        co2_data (pd.DataFrame): DataFrame with CO2 concentrations
        event (Dict): Dict with injection event timing
        result (Dict): Dict with analysis results
    ...
    if not np.isnan(lambda_value):
        ...
        if len(decay_data) > 0:
            c_0 = decay_data["C_bedroom"].iloc[0]
    ...
    # Title
    title = f"Event {event_number}: CO$_2$ Decay Analysis"
    ...
    # -------------------------------------------------------------------------
    # Bottom panel: Linearized plot (y vs t)
    # -------------------------------------------------------------------------
    ...
    if y_values and t_values and not np.isnan(lambda_value):
        ...
        ax2.plot(
            ...
            color=COLORS["fit"],
            linewidth=2.0,
            label=f"λ = {lambda_value:.4f} h⁻¹ (R² = {r_squared:.4f})",
        )
