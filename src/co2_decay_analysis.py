...
def load_and_merge_co2_data() -> pd.DataFrame:
    ...
    for col in co2_cols:
        if col in merged.columns:
            merged[col] = (
                merged[col].rolling(window=ROLLING_WINDOW_MIN, center=True, min_periods=1).mean()
            )
...
def calculate_dynamic_decay_end(
    ...
    mask = (co2_data["datetime"] >= decay_start) & (co2_data["datetime"] <= max_decay_end)
    ...
    if below_threshold.any():
        ...
        decay_end_series = decay_data.loc[decay_data.index == first_below_idx, "datetime"]
...
def run_co2_decay_analysis(
    ...
    for event in events:
        event["decay_end"] = calculate_dynamic_decay_end(
            co2_data,
            event["decay_start"],
            event["decay_end"],  # This is the max (2-hour) end from identify_injection_events
