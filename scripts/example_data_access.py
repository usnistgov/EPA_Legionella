...
def main():
    ...
    if outdoor_file.exists():
        ...
        print(
            f"Wind speed range: {df_outdoor['Wind_Speed_m/s'].min():.1f} - {df_outdoor['Wind_Speed_m/s'].max():.1f} m/s"
        )
        print(
            f"Temperature range: {df_outdoor['Ambient_Temperature_degC'].min():.1f} - {df_outdoor['Ambient_Temperature_degC'].max():.1f} °C"
        )
    else:
