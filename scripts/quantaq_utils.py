...
class QuantAQAPI:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Get API key
        if api_key is None:
            env_var = self.config.get("api_config", {}).get("env_var_api_key", "QUANTAQ_API_KEY")
    ...
    def save_data_to_file(
        ...
        filename = f"{serial_number}_{data_type}{date_range}_{timestamp}.{output_format}"
...
def interactive_main():
    ...
    try:
        ...
        print(f"[OK] Connected to QuantAQ API")
    ...
    list_devices = input("\nWould you like to list available devices first? (y/n): ").strip().lower()
    if list_devices == 'y':
        print("\nFetching devices...")
    ...
    if data:
        ...
        if isinstance(data, dict) and 'data' in data:
            ...
            # Ask to save
            save = input("\nWould you like to save the data to a file? (y/n): ").strip().lower()
            if save == 'y':
                fmt = input("Output format (csv/xlsx, default: csv): ").strip().lower() or "csv"
                filepath = quantaq_api.save_data_to_file(
                    serial_number, data_type, 
