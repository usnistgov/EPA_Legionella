...
try:
    from scripts.data_paths import get_instrument_path, get_instrument_config
except ImportError:
    # Fallback if running from different location
    from data_paths import get_instrument_path, get_instrument_config
...
def organize_expanded_columns(df: pd.DataFrame, prefix: str) -> Tuple[pd.DataFrame, List[str]]:
    """
    ...
    bin_cols = sorted([col for col in prefix_cols if '_bin' in col], 
                      key=lambda x: int(re.search(r'bin(\d+)', x).group(1)) if re.search(r'bin(\d+)', x) else 0)
    pm_cols = [col for col in prefix_cols if '_pm' in col]
    other_cols = [col for col in prefix_cols if col not in bin_cols and col not in pm_cols]
...
def main():
    ...
    if not file_pairs:
        ...
        print(f"Looking for files matching pattern: *-raw.csv and *-final.csv")
